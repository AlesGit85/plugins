...
class PodlahoveVytapeniKalkulator {
        ...
        foreach ($floors as $index => $floor) {
            ...
            if ($index === 0) {
                switch ($floor['heat_source']) {
                    case 'low_temp':
                        $floor_cost += $settings['low_temp_source_price'];
                        break;
                    case 'high_temp':
                        $floor_cost += $settings['high_temp_source_price'];
                        break;
                    case 'radiator_combo':
                        $floor_cost += $settings['radiator_combo_price'];
                        break;
                }
            }
    ...
    public function ajax_send_calculation_email() {
        ...
        $contact_support = isset($_POST['contact_support']) ? true : false;
